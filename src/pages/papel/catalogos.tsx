  ...
  interface TabConfig {
    ...
    // ✅ NUEVO — catálogos unificados con Proveedores/Insumos (muestran
    // proveedores, código y precio, igual que Foil). El tipo de insumo queda
    // implícito por la pestaña; el usuario no elige tipo en ningún momento.
    esInsumo?: boolean;
  ...
  const TAB_GROUPS: TabGroup[] = [
    ...
    },
    {
      ...
      tabs: [
        // ✅ Unificados con insumos — la medida se guarda como parte del nombre
        // (ej. "Sacabocado 3 mm"), por eso llevan conMedida + prefijoNombre.
        { key: "sacabocados", label: "Sacabocados", hasMedida: true, icon: "⭕", esInsumo: true, tipoInsumoNombre: "Sacabocados", conMedida: true, prefijoNombre: "Sacabocado", placeholderMedida: "Ej. 3 mm" },
        { key: "perforado", label: "Perforado", hasMedida: true, icon: "🔵", esInsumo: true, tipoInsumoNombre: "Perforado", conMedida: true, prefijoNombre: "Perforación", placeholderMedida: "Ej. 6.35x6.35 mm" },
        { key: "matrix", label: "Matrix", hasMedida: false, icon: "🔲", esInsumo: true, tipoInsumoNombre: "Matrix" },
  ...
  export default function Catalogos() {
    ...
    // se cargan aparte leyendo directo de cat_* (sin el LIMIT 50 de buscarInsumos).
    ...
    const cargarConteoInsumo = useCallback(async (catKey: CatKey) => {
      try {
        const rows = await getCatalogoInsumo(catKey as CatKeySincronizado, true);
        setInsumoCounts((prev) => ({ ...prev, [catKey]: rows.length }));
      } catch {
        // si falla, simplemente no se muestra badge para esa pestaña
      }
    }, []);
    ...
    // ✅ NUEVO — detecta si la pestaña activa es uno de los 6 catálogos
          ...
          <div style={{ display: "grid", gridTemplateColumns: "280px 1fr", gap: 24, alignItems: "start" }}>
            ...
            <main style={{ background: "#fff", border: "1px solid #E5E7EB", borderRadius: 12, padding: 24, boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
              ...
              {activeTab === "foil" ? (
                ...
                <InsumoCatalogoPanel
                  tipoInsumoNombre={activeTabConfig.tipoInsumoNombre!}
                  conMedida={activeTabConfig.conMedida}
