...
function Btn({ children, onClick, variant = "primary", small }: {
  ...
  return (
    <button onClick={onClick} style={{
      height: small ? 30 : 38, padding: small ? "0 12px" : "0 18px",
      borderRadius: 7, cursor: "pointer", fontSize: small ? 12 : 13,
      fontWeight: 600, display: "inline-flex", alignItems: "center",
      gap: 6, whiteSpace: "nowrap", ...styles[variant],
    }}>
      {children}
    </button>
  );
...
function SacabocadosInput({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div style={{ display: "flex", height: 38, border: "1px solid #D1D5DB", borderRadius: 7, overflow: "hidden", background: "#fff" }}>
      <input
        type="text" inputMode="numeric" pattern="[0-9]*" placeholder="ej: 3"
        value={num}
      ...
      />
      <span style={{ height: "100%", padding: "0 10px", borderLeft: "1px solid #D1D5DB", background: "#F3F4F6", fontSize: 13, fontWeight: 600, color: "#374151", display: "flex", alignItems: "center" }}>
        mm
      </span>
    </div>
  );
...
function PerforadoInput({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div style={{ display: "flex", height: 38, border: "1px solid #D1D5DB", borderRadius: 7, overflow: "hidden", background: "#fff" }}>
      <input
        type="text" inputMode="decimal" placeholder="ej: 6.35x6.35"
        value={val}
        onChange={e => {
          const cleaned = e.target.value.replace(/[^0-9.xX]/g, "").toUpperCase();
          setVal(cleaned); update(cleaned);
        }}
        style={{ flex: 1, height: "100%", padding: "0 10px", border: "none", fontSize: 13, color: "#111827", outline: "none", background: "transparent" }}
      />
      <span style={{ height: "100%", padding: "0 10px", borderLeft: "1px solid #D1D5DB", background: "#F3F4F6", fontSize: 13, fontWeight: 600, color: "#374151", display: "flex", alignItems: "center" }}>
...
function CalibreInput({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div style={{ display: "flex", height: 38, border: "1px solid #D1D5DB", borderRadius: 7, overflow: "hidden", background: "#fff" }}>
      <input
        type="text" inputMode="numeric" pattern="[0-9]*" placeholder="ej: 14"
        value={num}
...
function CatPanel({ tab, items, onAdd, onEdit, onDelete, onReactivar, verInactivos, loadingInactivos }: {
  ...
  const handleEdit = async () => {
    ...
    if (esFijo) {
      ...
      const medidaFinal = editMedida.trim().toLowerCase().endsWith("mm")
        ? editMedida.trim()
        : `${editMedida.trim()} mm`;
      setSaving(true);
    ...
    try {
      await onEdit(
        editId,
        editNombre.trim(),
        tab.hasMedida ? editMedida.trim() : undefined,
        tab.tieneNumMaquina ? editNumMaquina.trim() : undefined,
      );
      setEditId(null);
    } finally { setSaving(false); }
  };
  ...
  const startEdit = (item: CatItem) => {
    ...
    const medidaNormalizada = (tab.key === "sacabocados" || tab.key === "perforado") && medida && !medida.toLowerCase().endsWith(" mm")
      ? `${medida} mm`
      : medida;
    ...
    <div>
      ...
      {!verInactivos && (
        <div style={{ background: "#F9FAFB", border: "1px dashed #D1D5DB", borderRadius: 9, padding: "16px 18px", marginBottom: 20 }}>
          ...
          <div style={{
            display: "grid",
            gridTemplateColumns: esFijoTab
              ? "1fr auto"
              : tab.tieneNumMaquina
                ? "1fr 120px auto"
                : tab.hasMedida ? "1fr 1fr auto" : "1fr auto",
            gap: "0 10px",
            alignItems: "end",
          }}>
            {/* Nombre — oculto para sacabocados/perforado */}
            {!esFijoTab && (
              <div>
                <label style={labelStyle}>{colLabel}</label>
                {tab.key === "calibre" ? (
                  <CalibreInput value={newNombre} onChange={setNewNombre} />
                ) : (
                  <Inp
                    placeholder={tab.key === "refuerzo_medidas" ? "ej: 10x10 cm" : "Nombre…"}
                    value={newNombre}
            ...
            {/* Medida (sacabocados / perforado usan input especial; otros usan Inp genérico) */}
            {(esFijoTab || tab.hasMedida) && (
              <div>
                <label style={labelStyle}>Medida</label>
                {tab.key === "sacabocados" ? (
                  <SacabocadosInput value={newNombre} onChange={setNewNombre} />
                ) : tab.key === "perforado" ? (
                  <PerforadoInput value={newNombre} onChange={setNewNombre} />
                ) : (
                  <Inp placeholder={placeholderMedida} value={newMedida} onChange={setNewMedida} />
                )}
              </div>
            )}
            ...
            {tab.tieneNumMaquina && (
              <div>
                <label style={labelStyle}>N° Máquina</label>
                <Inp placeholder="ej: 1.2" value={newNumMaquina} onChange={setNewNumMaquina} />
              </div>
            )}
      ...
      {verInactivos && (
        <div style={{ background: "#FEF2F2", border: "1px dashed #FECACA", borderRadius: 9, padding: "12px 16px", marginBottom: 20, display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <p style={{ margin: 0, fontSize: 12, color: "#DC2626", fontWeight: 500 }}>
            {loadingInactivos ? "Cargando registros inactivos…" : "Registros desactivados — puedes reactivarlos para volver a usarlos en el formulario."}
          </p>
        </div>
      )}
      ...
      <div style={{ border: "1px solid #E5E7EB", borderRadius: 9, overflow: "hidden" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: gridCols, background: "#F9FAFB", borderBottom: "1px solid #E5E7EB", padding: "0 16px" }}>
          {[
            "#",
            colLabel,
            ...(tab.hasMedida ? ["Medida"] : []),
            ...(tab.tieneNumMaquina ? ["N° Máquina"] : []),
            "",
          ].map((h, i) => (
        ...
        {filtered.length === 0 ? (
          ...
          <div
            key={item.id}
            style={{
              display: "grid", gridTemplateColumns: gridCols, padding: "0 16px",
          ...
          >
            ...
            {tab.hasMedida && (
              editId === item.id
                ? tab.key === "sacabocados"
                  ? <SacabocadosInput value={editMedida} onChange={setEditMedida} />
                  : tab.key === "perforado"
                    ? <PerforadoInput value={editMedida} onChange={setEditMedida} />
                    : <Inp value={editMedida} onChange={setEditMedida} placeholder={placeholderMedida} style={{ height: 32, fontSize: 13 }} />
            ...
            {tab.tieneNumMaquina && (
              editId === item.id
                ? <Inp value={editNumMaquina} onChange={setEditNumMaquina} placeholder="ej: 1.2" style={{ height: 32, fontSize: 13 }} />
                : <span style={{ fontSize: 13, color: "#6B7280" }}>{item.numero_maquina ?? "—"}</span>
            )}
        ...
        {sections.map(s => {
          ...
          return (
            <button key={s.key} onClick={() => setActiveSection(s.key)}
              style={{ flex: 1, height: 42, border: "none", cursor: "pointer", background: active ? "#1D4ED8" : "#F9FAFB", color: active ? "#fff" : "#6B7280", fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", gap: 7, borderRight: s.key === "medidas" ? "1px solid #E5E7EB" : "none", transition: "background 0.15s" }}>
        ...
        <div style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
                ...
                {group.tabs.map(tab => {
                  ...
                  return (
                    <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                      style={{ width: "100%", padding: "10px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, border: "none", borderLeft: active ? "3px solid #1D4ED8" : "3px solid transparent", background: active ? "#EFF6FF" : "#fff", cursor: "pointer", borderBottom: "1px solid #F3F4F6", transition: "background 0.1s", textAlign: "left" }}>
                      ...
                      <span style={{ fontSize: 11, fontWeight: 600, color: active ? "#1D4ED8" : "#9CA3AF", background: active ? "#DBEAFE" : "#F3F4F6", borderRadius: 10, padding: "1px 7px", flexShrink: 0 }}>{count}</span>
                    </button>
                  );
                })}
              </div>
          ...
          <div style={{ flex: 1, background: "#fff", border: "1px solid #E5E7EB", borderRadius: 10, padding: "20px 22px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20, paddingBottom: 14, borderBottom: "1px solid #E5E7EB" }}>
              <span style={{ fontSize: 22 }}>{activeTab === "refuerzoBase" ? "🔩" : activeTabConfig?.icon}</span>
              <div>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", marginBottom: 16, gap: 8 }}>
              <span style={{ fontSize: 12, color: "#6B7280", fontWeight: 500 }}>
                {verInactivos ? "Mostrando inactivos" : "Mostrar inactivos"}
              </span>
              <button
                type="button"
            ...
            {activeTab === "refuerzoBase" ? (
              <RefuerzoBasePanel
                catalogs={verInactivos ? catalogsInactivos : catalogs}
                addItem={addItem}
                editItem={editItem}
