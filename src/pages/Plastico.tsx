...
//           placeholder="Buscar por tipo, material, medida o calibre..."
...
//         <div className="grid gap-2 bg-gray-50 border-b border-gray-200 px-4" style={{ gridTemplateColumns: "1.3fr 1fr 0.8fr 0.8fr 1.2fr 130px auto" }}>
//           {["Tipo de producto", "Material", "Calibre", "Medida", "Bolsas/kilo", "Archivos", ""].map((h) => (
//             <div key={h} className="py-2 text-[10px] font-bold tracking-wider uppercase text-gray-500">{h}</div>
//           ))}
//         </div>
...
//               style={{
//                 gridTemplateColumns: "1.3fr 1fr 0.8fr 0.8fr 1.2fr 130px auto",
...
//                 </div>
//                 <p className="text-[10px] text-gray-400">ID #{p.id}{p.identificador ? ` — ${p.identificador}` : ""}</p>
//               </div>
//               <span className="text-sm text-gray-700 truncate">{p.material}</span>
//               <span className="text-sm text-gray-700">{formatNumero(p.calibre)}</span>
//               <span className="text-sm text-gray-700 truncate">{p.medida}</span>
//               <span className="text-sm text-gray-700">{formatNumero(p.por_kilo)}</span>
...
//     calibre: formatNumero(d.calibre),
        ...
        {loading ? (
          ...
          filtered.map((p, idx) => (
            ...
            >
              ...
              <span className="text-sm text-gray-700 truncate">{p.material || "—"}</span>
              <span className="text-sm text-gray-700">{p.calibre != null ? formatNumero(p.calibre) : "—"}</span>
              <span className="text-sm text-gray-700 truncate">{p.medida || "—"}</span>
              <span className="text-sm text-gray-700">{formatNumero(p.por_kilo)}</span>
              <div className="flex gap-1">
                {p.archivos_preview && p.archivos_preview.length > 0 ? (
