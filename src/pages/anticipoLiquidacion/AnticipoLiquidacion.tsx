...
import { generarPdfEstadoCuenta } from "../../utils/generarPdfEstadoCuenta";
import { generarPdfEstadoCuentaSimple } from "../../utils/generarPdfEstadoCuentaSimple";
import { generarPdfHistorialPagos } from "../../utils/generarPdfHistorialPagos";
...
// ── Porcentaje de referencia visual ──────────────────────────────────────────
// El backend activa producción al 40% del total.
// El anticipo guardado en BD (y que ve el cliente) es el 50%.
// En el frontend solo usamos ANTICIPO_VALIDACION_MIN para dibujar la barra.
...
// sea EN_PROCESO aunque el abono sea 0 si el crédito fue autorizado.
// Usa anticipo (50% de BD) como umbral visual.
// ─────────────────────────────────────────────────────────────────────────────
function calcularEstado(
  abono: number,
  anticipo: number,
  saldo: number,
  esCreditoAnticipo: boolean = false
): number {
  if (saldo <= 0.01)                          return ESTADO.PAGADO;
  if (abono >= anticipo || esCreditoAnticipo) return ESTADO.EN_PROCESO;
  ...
  const cargar = async () => {
    ...
    try {
      ...
      onActualizar({
        ...
        es_credito_anticipo: data.es_credito_anticipo,
      });
    } catch (e: any) {
      ...
      <div>
        ...
        <p className="text-[10px] text-gray-400">
          ...
          {esKiloProd && <span className="ml-1.5 inline-flex items-center px-1 py-0.5 rounded text-[9px] font-semibold bg-orange-100 text-orange-700">Por kg</span>}
                      ...
                      {prod.herramental_aprobado === true && prod.herramental_precio != null && prod.herramental_precio > 0 && (
                          ...
                          <div className="flex items-center gap-2 px-3 py-2">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {prod.herramental_descripcion && (
                                <span className="text-xs text-orange-700 ml-1 font-medium">
                                  — {prod.herramental_descripcion}
                                </span>
                              )}
                            </div>
                            <span className="text-xs font-bold text-orange-800 flex-shrink-0">
                              +${fmt(prod.herramental_precio)}
                            </span>
                          </div>
                          <div className="px-3 pb-2.5 border-t border-orange-100 pt-1.5">
                            <p className="text-[10px] text-orange-700 leading-relaxed">
                              <span className="font-semibold">¿Por qué se cobra?</span>{" "}
                              El herramental es el costo único de fabricación del molde o troquel necesario
                              para producir este artículo. Se cotiza por separado y{" "}
                              <span className="font-semibold">no forma parte del precio por pieza</span>.
                              En pedidos futuros del mismo artículo, este cargo no se repite.
                            </p>
                          </div>
                        </div>
                      )}
              ...
              <div className="grid grid-cols-2 gap-2">
                <button onClick={handleDescargarSimple} disabled={generandoSimple}
                ...
                <button onClick={handleDescargarDetalle} disabled={generandoDetalle}
...
export function EditarAntLiqReal({
  ...
  // Leer es_credito_anticipo del objeto venta
  const esCreditoAnticipo = !!(venta as any).es_credito_anticipo;

  // ── Solo para la barra visual: dónde cae el 40% en la barra ──────────────
  const umbralActivacion = Number((Number(venta.total) * ANTICIPO_VALIDACION_MIN).toFixed(2));

  // ── Lógica de estado visual — siempre contra el 50% (anticipo de BD) ─────
  // anticreditoActivo: producción arrancó pero el cliente aún no cubre el 50%
  const anticreditoActivo =
    Number(venta.estado_id) >= ESTADO.EN_PROCESO &&
    totalPagado < anticipo - 0.01;

  // anticipoCubierto: el cliente llegó al 50% oficial O fue por crédito
  ...
  // pagado usa calcularEstado con esCreditoAnticipo y anticipo del 50%
  const pagado = calcularEstado(totalPagado, anticipo, saldo, esCreditoAnticipo) === ESTADO.PAGADO;
    ...
    <div className="space-y-6">
          ...
          <div className="bg-orange-50 border border-orange-200 rounded-lg overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2.5">
              <div className="flex items-center gap-2">
                <span className="text-base flex-shrink-0">🔧</span>
                <div>
                  <span className="text-xs font-semibold text-orange-800">Herramental aprobado</span>
            ...
            <div className="px-4 pb-3 border-t border-orange-100 pt-2">
              <p className="text-[10px] text-orange-700 leading-relaxed">
                <span className="font-semibold">¿Qué es este cargo?</span>{" "}
                El herramental es el costo único de fabricación del molde o troquel requerido
                para producir los artículos de este pedido. Se cobra una sola vez y{" "}
                <span className="font-semibold">no se incluye en el precio unitario por pieza</span>.
                Si en el futuro se reordena el mismo artículo, este cargo no aplica nuevamente.
              </p>
            </div>
          </div>
        )}
      ...
      <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
        ...
        <div className="w-full bg-gray-200 rounded-full h-3 relative">
          {/* Línea azul en 40% — referencia visual de activación de producción */}
          <div className="absolute top-0 bottom-0 w-0.5 bg-blue-400 z-10" style={{ left: "40%" }} />
          {/* Línea naranja en 50% — anticipo oficial que ve el cliente */}
          <div className="absolute top-0 bottom-0 w-0.5 bg-orange-400 z-10" style={{ left: "50%" }} />
        ...
        <p className="text-center text-xs mt-1 text-blue-500">
          {pagado
            ? "✓ Pedido pagado"
            : esCreditoAnticipo && !anticreditoActivo
              ? "🤝 Anticipo a crédito autorizado — producción en curso"
              : anticreditoActivo
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          {(venta.pagos?.length ?? 0) > 0 && (
            <button onClick={handleDescargarHistorial} disabled={descargandoHist}
              ...
              <div key={pago.idventa_pago}
                ...
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                    pago.es_credito_anticipo ? "bg-amber-100 text-amber-700"
                    : pago.es_anticipo       ? "bg-blue-100 text-blue-700"
                    :                          "bg-emerald-100 text-emerald-700"
                  }`}>
                    {pago.es_credito_anticipo ? "🤝" : pago.es_anticipo ? "A" : "$"}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {pago.es_anticipo && !pago.es_credito_anticipo && (
      ...
      {!pagado ? (
        <div className="bg-gray-50 rounded-xl border border-gray-200 p-4">
          <h4 className="text-sm font-semibold text-gray-700 mb-4">
            {esCreditoAnticipo && totalPagado < anticipo - 0.01
              ...
              <div className="relative">
                ...
                <input type="text" inputMode="decimal" value={monto} readOnly={montoEsAnticipo}
                  onChange={e => {
                    ...
                    if (/^\d*\.?\d{0,2}$/.test(val)) {
                      setMonto(val); setEsAnticipo(false); setMontoEsAnticipo(false);
                    }
                  }}
                  placeholder="0.00"
          ...
          <div className="flex items-center justify-between">
            {!anticipoCubierto ? (
              ...
              <span className="text-xs text-emerald-600 font-medium">
                {esCreditoAnticipo ? "🤝 Anticipo a crédito autorizado" : "✓ Anticipo cubierto"} — este pago es abono normal
              </span>
            )}

            <div className="flex items-center gap-2">
              {/* Solo mostrar si NO es ya crédito Y NO está cubierto */}
              {!anticipoCubierto && !esCreditoAnticipo && (
                <button type="button" onClick={handleAutorizarCredito} disabled={autorizando}
              ...
              <button onClick={handleRegistrarPago} disabled={guardando || !monto}
                ...
                {guardando
                  ...
                  : <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>Registrar pago</>
                }
              </button>
            </div>
          </div>
...
export default function AnticipoLiquidacion() {
  ...
  const getEstadoBadge = (v: Venta) => {
    ...
    const esCreditoAnticipo = !!(v as any).es_credito_anticipo;
    const estado = calcularEstado(abono, anticipo, saldo, esCreditoAnticipo);
    ...
    if (estado === ESTADO.EN_PROCESO) return (
      <div className="flex flex-col gap-1">
        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
          {esCreditoAnticipo ? "🤝 A crédito" : "🔄 En proceso"}
        </span>
