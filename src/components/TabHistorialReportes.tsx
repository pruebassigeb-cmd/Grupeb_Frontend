...
function SeccionRemisiones() {
  ...
  const expandirCliente = async (cliente: ClienteRemision) => {
    if (clienteExpandido === cliente.idclientes) {
      setClienteExpandido(null); setPedidosCliente([]); setPedidosSeleccionados([]);
      setVistaHistorial(false); setHistorial([]); setClienteActivo(null);
      return;
    }
    setClienteExpandido(cliente.idclientes);
    setClienteActivo(cliente);
    setPedidosSeleccionados([]); setVistaHistorial(false); setHistorial([]);
    setLoadingPedidos(true);
    try { setPedidosCliente(await getPedidosClienteRemisiones(cliente.idclientes)); }
    catch { showAlert("Error al cargar pedidos del cliente"); }
    finally { setLoadingPedidos(false); }
  };

  const togglePedido = (id: number) =>
    setPedidosSeleccionados(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);
  ...
  const verHistorial = async () => {
    ...
    } catch { showAlert("Error al cargar historial de entregas"); }
    finally { setLoadingHistorial(false); }
  };

  const handlePDF = async () => {
    if (!historial.length || !clienteActivo) return;
    await generarReporteRemisiones({ cliente: clienteActivo.nombre, pedidos: pedidosCliente, historial, pedidosSeleccionados });
  };
  ...
  const descargarNota = async (entrega: any) => {
    ...
    } finally { setDescargandoNota(null); }
  };

  // buildGuiaObs ya no se usa para obs de entrega, se elimina
  const buildGuiaObs = (e: any) => e.numero_guia ? [e.numero_guia] : [];
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                      {["#", "Fecha", "Nota / Remisión", "Tipo", "Producto(s)", "Bultos", "Cantidad", "Guía", "Estado"].map(h => (
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {hEntregas.entregas.length === 0 ? (
                      ...
                      const guiaObs = buildGuiaObs(e);

                      return prods.map((p: any, pIdx: number) => {
                        const esFirst = pIdx === 0;
                        return (
                          <tr key={`${e.idenvio}-${idx}-${pIdx}`} className="hover:bg-gray-50">
                              ...
                              {esFirst ? (
                                e.nota_no ? (
                                  <>
                                    ...
                                    {e.observaciones && (
                                      <p className="text-xs text-amber-600 italic mt-0.5 leading-tight" style={{ maxWidth: "190px" }}>
                                        {e.observaciones}
                                      </p>
                                    )}
                                  </>
                            ...
                            <td className="px-3 py-2">
                              {esFirst ? (
                                e.numero_guia
                                  ? <span className="font-mono text-gray-700 text-xs">{e.numero_guia}</span>
                                  : <span className="text-gray-400">—</span>
                              ) : null}
                            </td>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {pedidosCliente.map(pedido => {
                      const sel = pedidosSeleccionados.includes(pedido.idsolicitud);
                      return (
                        <div key={pedido.idsolicitud} onClick={() => togglePedido(pedido.idsolicitud)}
                          className={`border rounded-lg p-3 cursor-pointer transition-colors ${sel ? "border-emerald-500 bg-emerald-50" : "border-gray-200 hover:border-gray-300"}`}>
                          <div className="flex items-center gap-3">
                            <div className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 ${sel ? "bg-emerald-500 border-emerald-500" : "border-gray-300"}`}>
                              {sel && (
  ...
  return (
    <div>
      <div className="mb-5">
        ...
        <p className="text-xs text-gray-500 mt-0.5">Consulta el historial de entregas y remisiones por cliente.</p>
      </div>
      <SeccionRemisiones />
    </div>
  );
