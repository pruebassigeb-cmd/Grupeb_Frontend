...
interface FormRegistrarInsumo {
  ...
  unidad: string;
  producto_sat_idproducto_sat: string | number;
}
...
export default function ModalRegistrarInsumo({
  ...
  useEffect(() => {
    const cargar = async () => {
      try {
        const [tiposResponse, proveedoresResponse, productosSatResponse] =
          await Promise.all([
            getTiposInsumo(),
            getProveedores(),
        ...
        setProductosSat(productosSatResponse);
      } catch {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(async () => {
      ...
      try {
        const resultados = await buscarInsumos(
          form.tipo_insumo_id,
          form.nombre.trim()
        );

        const nombreBuscado = form.nombre
          .trim()
          .toLowerCase();

        const exacto = resultados.find(
          (resultado) =>
            resultado.nombre.toLowerCase() === nombreBuscado
        );
        ...
        const nombresProveedores = exacto.proveedores
          .map((proveedor) => proveedor.proveedor_nombre)
          .join(", ");
  ...
  const setCampo = <K extends keyof FormRegistrarInsumo>(
    campo: K,
    valor: FormRegistrarInsumo[K]
  ) => {
    setForm((prev) => ({
      ...prev,
      [campo]: valor,
  ...
  const handlePrecioChange = (valor: string) => {
    if (!/^\d*\.?\d{0,4}$/.test(valor)) {
      return;
    }

    setCampo("precio", valor);
  };

  const handleMinimoChange = (valor: string) => {
    if (!/^\d*\.?\d{0,2}$/.test(valor)) {
      return;
    }

    setCampo("minimo_compra", valor);
  };
  ...
  const handleGuardar = async () => {
    ...
    if (duplicado) {
      showAlert(
        "Corrige el duplicado antes de guardar",
        "warning"
      );
      return;
    }
    ...
    try {
      const resultado = await registrarInsumoRapido({
        ...
        precio: form.precio.trim()
          ? Number.parseFloat(form.precio)
          : null,
        notas: form.notas.trim() || null,
        clave_producto:
          form.clave_producto.trim() || null,
        minimo_compra: form.minimo_compra.trim()
          ? Number.parseFloat(form.minimo_compra)
          : null,

        // Ahora el DTO acepta string | null.
        unidad: form.unidad.trim() || null,

        producto_sat_idproducto_sat:
          form.producto_sat_idproducto_sat
            ? Number(form.producto_sat_idproducto_sat)
            : null,
      });

      showAlert(
        `Insumo "${resultado.nombre}" registrado`,
        "success"
      );
  ...
  const inputClass =
    "w-full px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-900 bg-white focus:ring-2 focus:ring-purple-500 focus:border-transparent";
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">
                Se guardará en el catálogo de proveedores
              </p>
            </div>
          </div>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              >
                {tipos.map((tipo) => (
                  <option
                    key={tipo.idtipo_insumo}
                    value={tipo.idtipo_insumo}
                  >
                    {tipo.nombre}
                  </option>
                ))}
            ...
            <div>
              ...
              <input
                ...
                value={form.codigo}
                onChange={(event) =>
                  setCampo("codigo", event.target.value)
                }
                placeholder="PMS-485, R-200..."
          ...
          <div>
            ...
            <input
              ...
              value={form.nombre}
              onChange={(event) =>
                setCampo("nombre", event.target.value)
              }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                value={form.notas}
                onChange={(event) =>
                  setCampo("notas", event.target.value)
                }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              >
                ...
                {productosSat.map((productoSat) => (
                  <option
                    key={productoSat.idproducto_sat}
                    value={productoSat.idproducto_sat}
                  >
                    {productoSat.clave} — {productoSat.pdft}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              <select
                value={form.unidad}
                onChange={(event) =>
                  setCampo("unidad", event.target.value)
                }
                className={selectClass}
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={(event) =>
                  handleMinimoChange(event.target.value)
                }
                placeholder="0.00"
                className={inputClass}
              />
          ...
          <div>
                ...
                proveedores.map((proveedor) => (
                  ...
                  >
                    <input
                      ...
                      onChange={() =>
                        toggleProveedor(proveedor.idproveedor)
                      }
            ...
            {proveedoresSeleccionados.length > 0 && (
              <p className="text-xs text-purple-600 mt-1 font-medium">
                ✓ {proveedoresSeleccionados.length} proveedor
            ...
            {proveedoresSeleccionados.length > 1 && (
              <p className="text-xs text-gray-400 mt-1">
                El código, precio, notas y mínimo de compra
                se guardarán igual para los{" "}
                {proveedoresSeleccionados.length} proveedores
                marcados.
              </p>
            )}
          </div>
          ...
          {duplicado && (
            <div className="flex items-start gap-2 px-3 py-2.5 bg-amber-50 border border-amber-200 rounded-lg">
              ...
              <p className="text-xs text-amber-700">
                Este insumo ya existe. Cambia el nombre o
                cancela y selecciónalo del dropdown.
              </p>
            </div>
          )}
        </div>
