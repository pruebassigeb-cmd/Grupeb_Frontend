...
import {
  ...
  getOrdenDisenoById,
  subirRevision,
} from "../../services/diseno/ordenDisenoService";
import api from "../../services/api";
import type {
  MensajeDiseno,
  OrdenDisenoDetalle,
  RevisionDiseno,
} from "../../types/diseno/ordenDiseno.types";
...
async function subirArchivoS3(file: File): Promise<number> {
  ...
  return data.id_archivo;
}

// ── Componente: muestra archivos de una revisión ──
function ArchivosRevision({ revision, esMio }: { revision: RevisionDiseno; esMio: boolean }) {
            ...
            {esImagen ? (
              ...
              <div className={`flex items-center gap-2 px-3 py-2 border rounded-xl transition-colors max-w-[200px] ${
                ...
                <span className={`text-xs truncate ${esMio ? "text-white" : "text-gray-700"}`}>
                  {archivo.nombre.length > 18 ? archivo.nombre.substring(0, 18) + "..." : archivo.nombre}
                </span>
              </div>
            ) : (
...
// ── Componente: mini modal para subir render o feedback ──
function SubirRevisionInline({
  idorden,
  noPedido,
  tipo,
  onSuccess,
  ...
  const handleSubir = async () => {
    if (archivos.length === 0) { showAlert("Selecciona al menos un archivo."); return; }
    setSubiendo(true);
    try {
      const ids = await Promise.all(archivos.map(subirArchivoS3));
      ...
      showAlert("No se pudo subir el archivo.");
    } finally {
      setSubiendo(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-600">
        {tipo === "render" ? "Sube el render del diseño para el pedido" : "Sube el feedback del cliente para el pedido"}{" "}
        <span className="font-semibold">{noPedido}</span>
      </p>
      ...
      <div onClick={() => fileRef.current?.click()}
        ...
        <p className="text-xs text-gray-400 mt-1">Imágenes o PDF</p>
      ...
      <div>
        ...
        <textarea rows={2} value={obs} onChange={e => setObs(e.target.value)}
          placeholder="Comentarios sobre esta versión..."
          className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" />
      </div>

      <div className="flex gap-2">
        <button onClick={onCancel} disabled={subiendo}
          className="flex-1 py-2 border border-gray-300 text-gray-700 rounded-lg text-sm hover:bg-gray-50 disabled:opacity-50">
          Cancelar
        </button>
        <button onClick={handleSubir} disabled={subiendo || archivos.length === 0}
          ...
          {subiendo
            ...
            : `Subir ${tipo === "render" ? "render" : "feedback"}`}
        </button>
      </div>
    </div>
  );
...
export default function ModalOrdenDiseno({ idorden_diseno, pedido, onClose }: Props) {
  ...
  const cargarOrden = useCallback(async () => {
    try {
      const data = await getOrdenDisenoById(idorden_diseno);
      setOrden(data);
  ...
  useEffect(() => { cargarOrdenRef.current = cargarOrden; }, [cargarOrden]);
    ...
    const poll = async () => {
      ...
      try {
        const desde = ultimoMsgRef.current
          ...
          : undefined;
        const nuevos = await getMensajes(idorden_diseno, desde);
        if (nuevos.length > 0) {
          const hayRevisionNueva = nuevos.some(
            (m: MensajeDiseno) =>
              m.tipo === "sistema" && m.revision_id && !revisionMapRef.current.has(m.revision_id)
          );
          if (hayRevisionNueva) { await cargarOrdenRef.current(); return; }
          setMensajes(prev => {
            const ids = new Set(prev.map((m: MensajeDiseno) => m.idmensaje));
            const filtrados = nuevos.filter((m: MensajeDiseno) => !ids.has(m.idmensaje));
  ...
  const handleEnviar = async () => {
    ...
    setEnviando(true);
    try {
      await enviarMensaje(idorden_diseno, texto.trim());
      setTexto("");
      const nuevos = await getMensajes(idorden_diseno);
      setMensajes(nuevos);
      if (nuevos.length > 0) ultimoMsgRef.current = nuevos[nuevos.length - 1].created_at;
    } catch (e: any) {
      showAlert(e.response?.data?.error || "Error al enviar mensaje");
    } finally {
      setEnviando(false);
    }
  };
  ...
  const handleAprobar = async () => {
    setAprobando(true);
    try {
      await aprobarOrdenDiseno(idorden_diseno);
  ...
  return (
    <div className="flex flex-col h-full" style={{ minHeight: 520 }}>
        ...
        <>
                ...
                <div key={msg.idmensaje}>
                  {!mismaFecha && (
                    <div className="flex items-center gap-2 my-2">
                      ...
                      <span className="text-xs text-gray-400 px-2">{fmtFecha(msg.created_at)}</span>
                      <div className="flex-1 h-px bg-gray-200" />
                    </div>
                  )}
                  ...
                  {revision && (
                    <div className={`flex ${revisionEsMia ? "justify-end" : "justify-start"} mt-1`}>
                      <div className="max-w-xs lg:max-w-sm">
                        <ArchivosRevision revision={revision} esMio={revisionEsMia} />
          ...
          {!aprobado && (
            <div className="flex gap-2 px-4 py-2 bg-white border-t border-gray-100 flex-wrap">
              {puedeEditarDiseno && (
                <button onClick={() => setModalSubir("render")}
              ...
              {puedeOrdenDiseno && !puedeEditarDiseno && (
                <button onClick={() => setModalSubir("feedback")}
          ...
          {orden.participantes.map(p => (
            <div key={p.idparticipante} className="flex items-center gap-3 bg-white border border-gray-200 rounded-xl px-4 py-3">
              <div className="w-9 h-9 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0">
                <span className="text-sm font-bold text-blue-700">{p.nombre.charAt(0).toUpperCase()}</span>
              </div>
      ...
      {modalSubir && (
        <Modal isOpen={!!modalSubir} onClose={() => setModalSubir(null)}
      ...
      {/* ── Modal aprobar ── */}
      {modalAprobar && (
        <Modal isOpen={modalAprobar} onClose={() => setModalAprobar(false)} title="Aprobar Diseño">
          <ModalAprobarDiseno
            noPedido={orden.no_pedido}
            versiones={versiones}
