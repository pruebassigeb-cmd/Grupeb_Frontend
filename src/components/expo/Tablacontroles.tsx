/**
 * TablaControles.tsx
 * Subcomponentes de la tabla de cotización:
 *   - Estilos compartidos (TH, TD, iSel, pill, …)
 *   - Estado global de dropdowns (openDrop / closeDrop / useOpenId)
 *   - BuscadorProductoModal
 *   - MedidaSelect, TintasSelect, CalibrePapelSelect,
 *     TintasPlasticoSelect, CantidadSelect
 *   - ExtraOPigmentoCell
 *   - FilaVacia
 *   - FilaTabla  ← componente principal que importa todo lo anterior
 */

import { useState, useRef, useCallback, memo, useEffect } from "react";
import {
  OPCIONES_LAMINADO, OPCIONES_HS, OPCIONES_TEXTURA, OPCIONES_ASA, OPCIONES_TINTAS,
  OPCIONES_TINTAS_PLASTICO, OPCIONES_TIPO_PLASTICO, MATERIALES_PLASTICO, CALIBRES_PLASTICO,
  MATERIALES_PAPEL, CALIBRES_PAPEL_FLAT, PIGMENTOS_PLASTICO,
  MEDIDAS_CAT, CATS,
} from "../../types/expo/expo.types";
import type { FilaProducto, Producto } from "../../types/expo/expo.types";
import { SISTEMA_PRODS } from "./Catalogo";
...
// ─── BuscadorProductoModal ─────────────────────────────────────────────────────
// Incluye un campo de nombre editable para los productos "en blanco": el
// vendedor puede dejarlo como "Producto nuevo" o escribir el nombre real
// antes de elegir la categoría.
interface BuscadorProps {
  catalogoPropio: Producto[];
  onElegir: (p: Producto) => void;
  onClose: () => void;
}
export function BuscadorProductoModal({ catalogoPropio, onElegir, onClose }: BuscadorProps) {
  ...
  const todos = [...catalogoPropio, ...SISTEMA_PRODS];
  ...
  const crearEnBlanco = (cat: "papel"|"plastico"|"carton") => {
    const vacio: Producto = {
      id: Date.now(), nombre: nombreNuevo.trim() || "Producto nuevo", categoria: cat,
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.75)", zIndex:300, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}
      ...
      <div style={{ background:"#1A1A1A", border:"1px solid #333", borderRadius:12, width:"100%", maxWidth:520, maxHeight:"80vh", display:"flex", flexDirection:"column", overflow:"hidden", boxShadow:"0 20px 60px rgba(0,0,0,.8)" }}
        ...
        {/* Producto en blanco — con nombre editable */}
        <div style={{ padding:"12px 16px 8px", borderBottom:"1px solid #2A2A2A" }}>
          <div style={{ color:"#666", fontSize:9, fontWeight:700, letterSpacing:1, textTransform:"uppercase", marginBottom:6 }}>O crear desde cero</div>
          <input
            value={nombreNuevo}
            onChange={e => setNombreNuevo(e.target.value)}
            placeholder="Producto nuevo"
            style={{ width:"100%", background:"#111", border:"1px solid #333", borderRadius:6, padding:"7px 10px", color:"#EEE", fontSize:12, outline:"none", fontFamily:"'Inter',sans-serif", marginBottom:8, boxSizing:"border-box" }}
          />
          <div style={{ display:"flex", gap:6 }}>
            {CATS.map(c => (
              <button key={c.key} onClick={() => crearEnBlanco(c.key as "papel"|"plastico"|"carton")}
                style={{ flex:1, background:`${c.color}14`, border:`1px solid ${c.color}55`, color:c.color, fontSize:10, fontWeight:700, padding:"7px 4px", borderRadius:6, cursor:"pointer" }}>
          ...
          {CATS.map(cat => {
            ...
            return (
              <div key={cat.key} style={{ marginBottom:8 }}>
                ...
                {ps.map(p => (
                  <div key={p.id} onClick={() => onElegir(p)}
...
// ─── CalibrePapelSelect ───────────────────────────────────────────────────────
interface CalibrePapelProps { id: string; value: string; onChange: (v: string) => void; }
export function CalibrePapelSelect({ id, value, onChange }: CalibrePapelProps) {
  const triggerRef = useRef<HTMLDivElement>(null);
  const inputRef   = useRef<HTMLInputElement>(null);
  const [busq, setBusq] = useState("");
  const [pos,  setPos]  = useState<DropPos | null>(null);
  ...
  const lista = busq.trim()
    ? CALIBRES_PAPEL_FLAT.filter(o => o.toLowerCase().includes(busq.trim().toLowerCase()))
    : CALIBRES_PAPEL_FLAT;
  const seleccionar = (v: string) => { onChange(v); closeDrop(); setBusq(""); };
  ...
  return (
    <>
      <div ref={triggerRef} onClick={abrir}
        style={{ ...iSel, fontSize:7.5, display:"flex", alignItems:"center", justifyContent:"center", gap:2, cursor:"pointer", padding:"2px 4px" }}>
        <span>{value || "Calibre"}</span><span style={{ fontSize:7 }}>▾</span>
      </div>
      {isOpen && pos && (
        <div id={`drop-${id}`} style={{ position:"fixed", top:pos.top, left:pos.left, width:100, background:"#1A1A1A", border:"1px solid #444", borderRadius:8, zIndex:9999, boxShadow:"0 8px 28px rgba(0,0,0,.7)", overflow:"hidden" }}>
          <div style={{ padding:"5px 6px 3px", borderBottom:"1px solid #2A2A2A" }}>
            <input ref={inputRef} value={busq} onChange={e => setBusq(e.target.value)} placeholder="ej: 150"
              style={{ width:"100%", background:"#111", border:"1px solid #333", borderRadius:4, padding:"4px 6px", color:"#EEE", fontSize:10, outline:"none", fontFamily:"'Inter',sans-serif" }}/>
          </div>
...
interface ExtraPigmentoProps {
  id: string;
  esPlastico: boolean;
  modo: "precio"|"pigmento";
  setModo: (m:"precio"|"pigmento") => void;
  extra: string;
  setExtra: (v:string) => void;
  pigmento: string;
  setPigmento: (v:string) => void;
}
export function ExtraOPigmentoCell({ id, esPlastico, modo, setModo, extra, setExtra, pigmento, setPigmento }: ExtraPigmentoProps) {
  const triggerRef = useRef<HTMLDivElement>(null);
  const [pos, setPos] = useState<DropPos | null>(null);
  const openId = useOpenId();
  const isOpen = openId === id;
  ...
  const esColorPredefinido = PIGMENTOS_PLASTICO.includes(pigmento as typeof PIGMENTOS_PLASTICO[number]);
  const mostrarComoOtro = eligioOtro || (pigmento !== "" && !esColorPredefinido);

  return (
    <div style={{ display:"flex", flexDirection:"column", gap:2, alignItems:"center", width:"100%", boxSizing:"border-box" }}>
      <div ref={triggerRef} onClick={abrirModo}
        title={modo==="pigmento" ? "Pigmento" : "Precio extra"}
      ...
      {isOpen && pos && (
        <div id={`drop-${id}`} style={{ position:"fixed", top:pos.top, left:pos.left, width:130, background:"#1A1A1A", border:"1px solid #444", borderRadius:8, zIndex:9999, boxShadow:"0 8px 28px rgba(0,0,0,.7)", overflow:"hidden" }}>
          ...
          {!esPlastico && (
            <div style={{ padding:"6px 12px", fontSize:9, color:"#444", fontStyle:"italic" }}>Pigmento solo aplica a plástico</div>
          )}
        </div>
      )}
      ...
      {modo === "precio" ? (
        <>
          <input style={{...iC,fontSize:8,color:extraNum?"#22C55E":"#999",width:"100%",boxSizing:"border-box"}}
            ...
            title="Cargo extra por pieza (se suma al precio)"
        ...
        <input
          autoFocus
          value={pigmento}
          onChange={e=>setPigmento(e.target.value)}
          onBlur={()=>{ if(!pigmento) setEligioOtro(false); }}
          placeholder="Color..."
        ...
        <select
          style={{...iSel,fontSize:7.5,width:"100%",boxSizing:"border-box"}}
          value={pigmento}
          onChange={e=>{
          ...
          {PIGMENTOS_PLASTICO.map(o=><option key={o} value={o}>{o}</option>)}
...
// ─── FilaVacia ─────────────────────────────────────────────────────────────────
interface FilaVaciaProps { onElegir: (p: Producto) => void; catalogoPropio: Producto[]; }
export function FilaVacia({ onElegir, catalogoPropio }: FilaVaciaProps) {
  const [open, setOpen] = useState(false);

  const vaciarYElegir = (p: Producto) => {
    const soloIdentidad: Producto = {
      id: p.id, nombre: p.nombre, categoria: p.categoria, imagen: p.imagen,
      medida: "", material: "", calibre: "", tintas: "",
      laminacion:false, hs:false, ar:false, textura:false, uv:false, asa:false, otro:"",
...
interface FilaProps {
  ...
  onEdit: (uid: string, k: keyof FilaProducto, v: string | boolean) => void;
  onEditNombre: (uid: string, nuevoNombre: string) => void;
}

export const FilaTabla = memo(function FilaTabla({ fila, rowIdx, onDel, onEdit, onEditNombre }: FilaProps) {
  const { uid, producto: p } = fila;
  const pre = `r${rowIdx}`;
  const esPlastico = p.categoria === "plastico";

  // Se inicializan desde `fila` (el snapshot que vive en el padre), no desde `p`
  // (el producto original), porque `fila` ya puede traer ediciones previas
  // (por ejemplo si la cotización fue cargada desde un registro guardado).
  ...
  const [matPlast,   setMatPlast]   = useState(esPlastico ? (fila.material || p.material || "") : "");
  const [calPlast,   setCalPlast]   = useState(esPlastico ? (fila.calibre  || p.calibre  || "") : "");
  const [tipoPlast,  setTipoPlast]  = useState(fila.tipoPlastico || p.tipoProducto || "");
  const [matPapel,   setMatPapel]   = useState(!esPlastico ? (fila.material || p.material || "") : "");
  const [calPapel,   setCalPapel]   = useState(!esPlastico ? (fila.calibre  || p.calibre  || "") : "");
  ...
  const propagar = useCallback((k: keyof FilaProducto, v: string|boolean) => onEdit(uid,k,v), [uid,onEdit]);
  ...
  const matCalPrint = esPlastico
    ? [matPlast, calPlast].filter(Boolean).join(" · ") || "—"
    : [matPapel, calPapel].filter(Boolean).join(" · ") || "—";
  const lamTipoPrint = esPlastico ? (tipoPlast || "—") : (laminacion ? tipoLam : "—");
  const hsPrint       = !esPlastico && hs ? tipoHs.replace("Hot stamping ","HS ") : "—";
  ...
  const extraPrint = modoExtra==="pigmento"
    ? (pigmento || "—")
    : (extraNum>0 ? `+$${extraNum}` : "—");

  return (
    <tr>
      {/* Nombre del producto — editable, especialmente útil para productos "en blanco" */}
      <td style={TDL}>
        <input
        ...
        <div className="no-print-show">
          {esPlastico ? (
            <div style={{ display:"flex", flexDirection:"column", gap:2 }}>
              <select style={{...iSel,fontSize:8}} value={matPlast}
                onChange={e=>{ setMatPlast(e.target.value); setCalPlast(""); propagar("material",e.target.value); propagar("calibre",""); }}>
                <option value="">Material</option>
                {MATERIALES_PLASTICO.map(o=><option key={o} value={o}>{o}</option>)}
              </select>
              <select style={{...iSel,fontSize:7.5}} value={calPlast} disabled={!matPlast}
                onChange={e=>{setCalPlast(e.target.value);propagar("calibre",e.target.value);}}>
                <option value="">{matPlast?"Calibre":"—"}</option>
                {(CALIBRES_PLASTICO[matPlast]||[]).map(o=><option key={o} value={o}>{o}</option>)}
              </select>
            </div>
          ) : (
            <div style={{ display:"flex", flexDirection:"column", gap:2 }}>
              <select style={{...iSel,fontSize:8}} value={matPapel}
                onChange={e=>{setMatPapel(e.target.value);propagar("material",e.target.value);}}>
                <option value="">Material</option>
                {MATERIALES_PAPEL.map(o=><option key={o} value={o}>{o}</option>)}
              </select>
              <CalibrePapelSelect id={`${pre}-cal`} value={calPapel} onChange={v=>{setCalPapel(v);propagar("calibre",v);}}/>
            </div>
          )}
        </div>
      ...
      {esPlastico ? (
        <>
          <td style={{...TD,fontSize:8}}>
            <select className="no-print-show" style={{...iSel,maxWidth:90}} value={tipoPlast}
              onChange={e=>{setTipoPlast(e.target.value);propagar("otro",e.target.value);propagar("tipoPlastico",e.target.value);}}>
              {OPCIONES_TIPO_PLASTICO.map(o=><option key={o} value={o==="Sin tipo"?"":o}>{o}</option>)}
            </select>
        ...
        <>
          <td style={{...TD,fontSize:8}}>
            <select className="no-print-show" style={{...iSel,maxWidth:82}} value={laminacion?tipoLam:""}
              onChange={e=>{const v=e.target.value;setLaminacion(v!=="");setTipoLam(v);propagar("laminacion",v!=="");propagar("tipoLaminado",v);}}>
          ...
          <td style={{...TD,fontSize:8}}>
            <select className="no-print-show" style={iSel} value={hs?tipoHs:""}
              onChange={e=>{const v=e.target.value;setHs(v!=="");setTipoHs(v);propagar("hs",v!=="");propagar("tipoHs",v);}}>
              {OPCIONES_HS.map(o=>(
                <option key={o} value={o==="Sin HS"?"":o}>
                  {o.replace("Hot stamping ","HS ")}
                </option>
              ))}
            </select>
          ...
          <td style={{...TD,fontSize:8}}>
            <select className="no-print-show" style={iSel} value={textura?tipoTex:""}
              onChange={e=>{const v=e.target.value;setTextura(v!=="");setTipoTex(v);propagar("textura",v!=="");propagar("tipoTextura",v);}}>
      ...
      <td style={{...TD,fontSize:8}}>
        <select className="no-print-show" style={iSel} value={asa?tipoAsa:""}
          onChange={e=>{const v=e.target.value;setAsa(v!=="");setTipoAsa(v);propagar("asa",v!=="");propagar("tipoAsa",v);}}>
      ...
      <td style={TDP}>
        <input className="no-print-show" style={iP} value={precio1} placeholder="$0.00" onChange={e=>setPrecio1(e.target.value)} onBlur={()=>propagar("precio1",precio1)}/>
        {extraNum>0&&<div className="no-print-show" style={{fontSize:7,color:"#22C55E",lineHeight:1}}>{conExtra(precio1,extraNum)}</div>}
        <span className="print-only">{extraNum>0 ? conExtra(precio1,extraNum) : precio1}</span>
      </td>

      <td style={TDP}>
        <input className="no-print-show" style={iP} value={precio2} placeholder="$0.00" onChange={e=>setPrecio2(e.target.value)} onBlur={()=>propagar("precio2",precio2)}/>
        {extraNum>0&&<div className="no-print-show" style={{fontSize:7,color:"#22C55E",lineHeight:1}}>{conExtra(precio2,extraNum)}</div>}
        <span className="print-only">{extraNum>0 ? conExtra(precio2,extraNum) : precio2}</span>
      </td>

      <td style={TDP}>
        <input className="no-print-show" style={iP} value={precio3} placeholder="$0.00" onChange={e=>setPrecio3(e.target.value)} onBlur={()=>propagar("precio3",precio3)}/>
        {extraNum>0&&<div className="no-print-show" style={{fontSize:7,color:"#22C55E",lineHeight:1}}>{conExtra(precio3,extraNum)}</div>}
        <span className="print-only">{extraNum>0 ? conExtra(precio3,extraNum) : precio3}</span>
      </td>
