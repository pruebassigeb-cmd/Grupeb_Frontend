...
export default function ModalMarcarRecoleccion({
  recoleccion,
  onClose,
  onSuccess,
  ...
  const handleSubmit = async () => {
    ...
    setGuardando(true);
    try {
      await marcarRecolectado(recoleccion.idenvio, {
        nombre_quien_recogio: form.nombre_quien_recogio.trim(),
        empresa: form.empresa.trim() || undefined,
        unidad_marca: form.unidad_marca.trim() || undefined,
        unidad_modelo: form.unidad_modelo.trim() || undefined,
        unidad_placas: form.unidad_placas.trim() || undefined,
        observacion_extra: form.observacion_extra.trim() || undefined, // ✅ Enviar
      });
      ...
      <div className="space-y-4">
        <div className="bg-purple-50 border border-purple-200 rounded-lg p-3 text-sm">
          <p className="font-semibold text-purple-800">
            {recoleccion.no_pedido} - {recoleccion.cliente}
          </p>
          <p className="text-purple-600 text-xs mt-0.5">
            {recoleccion.total_bultos} bulto(s)
          </p>
        </div>
        ...
        <div>
          ...
          <input
            type="text"
            value={form.nombre_quien_recogio}
            onChange={(e) =>
              setForm({ ...form, nombre_quien_recogio: e.target.value })
            }
        ...
        <div className="border border-gray-200 rounded-lg p-3 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                type="text"
                value={form.unidad_marca}
                onChange={(e) =>
                  setForm({ ...form, unidad_marca: e.target.value })
                }
                className={inputClass}
            ...
            <div>
              ...
              <input
                type="text"
                value={form.unidad_modelo}
                onChange={(e) =>
                  setForm({ ...form, unidad_modelo: e.target.value })
                }
                className={inputClass}
                placeholder="Transit, Ranger..."
              />
            </div>
          </div>
          <div>
            ...
            <input
              type="text"
              value={form.unidad_placas}
              onChange={(e) =>
                setForm({
                  ...form,
                  unidad_placas: e.target.value.toUpperCase(),
                })
              }
              className={inputClass}
              placeholder="ABC-123"
        ...
        {/* ✅ NUEVO CAMPO: Observaciones de entrega */}
        <div>
          <label className={labelClass}>Observaciones de entrega</label>
          <input
            type="text"
            ...
            className={inputClass}
            placeholder="Notas u observaciones de la entrega..."
          />
        </div>
        ...
        <div>
          ...
          {preview ? (
            ...
            <button
              ...
              className="w-full border-2 border-dashed border-gray-300 rounded-lg py-6 text-sm text-gray-500 hover:border-purple-400 hover:text-purple-500 transition-colors flex flex-col items-center gap-2"
            >
              <svg
                className="w-8 h-8"
                fill="none"
