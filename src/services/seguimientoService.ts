...
export interface NuevoBultoBatchPayload extends NuevoBultoPayload {
  repeticiones: number;
}

export interface BultoBatchRespuesta {
  bultos: Bulto[];
  repeticiones: number;
  total_bultos_agregados: number;
...
export const agregarBultosBatch = async (
  idproduccion: number,
  payload: NuevoBultoBatchPayload
): Promise<BultoBatchRespuesta> => {
  const { data } = await api.post(`/seguimiento/${idproduccion}/bultos/batch`, payload);
  return data;
};
...
export interface EtiquetaData {
  ...
  es_parcialidad: boolean;
  numero_envio_parcial: number | null;
}

export const getBultosEtiqueta = async (
  idproduccion: number,
...
export const marcarBultosParcialidad = async (
  idproduccion: number,
  numero_parcialidad: number,
  idbultos: number[]
): Promise<void> => {
  await api.patch(`/seguimiento/${idproduccion}/bultos/marcar-parcialidad`, {
    numero_parcialidad,
    idbultos,
  });
};